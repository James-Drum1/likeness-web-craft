...
// Generate PNG QR code as data URL that browsers can handle properly
function generatePNGQRCode(text: string, size: number = 300): string {
  ...
  // Create canvas-like data for PNG
...
// Fallback SVG method for server-side generation (since canvas is not available)
...
serve(async (req) => {
  ...
  try {
    ...
    for (const code of codes) {
      ...
      try {
        // Generate QR code as SVG (server-side, no canvas available)
